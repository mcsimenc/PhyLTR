...
class GFF3_line:
    ...
    Methods: str()              prints gff line
             refreshAttrStr()   updates gff line attributes 
                                (needed if changes to attributes 
                                were made)
    ...
    def __init__(self, line=None, **kwargs):
        ...
        if line == None:
            ...
            self.attributes_order = [ attr.split('=')[0] 
                                        for attr in attributes_list ]
            self.attributes = { attr.split('=')[0]:attr.split('=')[1] 
                                                  for attr in attributes_list }
        ...
        # rename the name attribute so it conforms to GFF3 
        # specifications, where Name is a reserved attribute key. 
        # The version of LTRDigest makes attribute key name
    ...
    def refreshAttrStr(self, attrOrder=None):
        ...
        If an attribute has been added it should have also been added
        to self.attributes_order

        """

        self.attributes_str = ';'.join(
                                    [ '='.join([ attr, self.attributes[attr] ]) 
...
def full2flankgff(inGFFpth, outGFFpth, bpflank):
    ...
    This function creates GFF3 files containing features of type
...
def getfasta(inGFFpth, fastaRefPth, outFastaPth, headerKey='ID'):
    """Function to run bedtools getfasta"""
...
def Circos(window='1000000', plots='clusters', I=6, clustering_method='WickerFam', WickerParams={'pId':80,'percAln':80,'minLen':80}, g='g0,g1,g2', MinCircosClusterSize=2):
    '''
    ...
    if clustering_method == 'WickerFam':
        WICKERCLUST = True
        ClustMethod = 'Wicker_{0}_pId_{1}_percAln_{2}_minLen'.format(WickerParams['pId'], WickerParams['percAln'], WickerParams['minLen'])
    elif clustering_method == 'MCL':
        MCLCLUST = True
...
auto_alpha_steps  = 5'''.format(imagesize)
                        ...
                        circoscallsScafs.append([circosdir, circos_call, classif, i, '{0}/plots.scaffolds'.format(paths['CurrentTopDir']), G])
...
auto_alpha_steps  = 5'''.format(imagesize)
                        ...
                        #circos_call = [executables['circos'], '-silent', '-conf', confbasename]
                        circos_call = [executables['perl'], executables['circos']]
                        circoscallsElmts.append([circosdir, circos_call, classif, i, '{0}/plots.elements'.format(paths['CurrentTopDir']), G])
        ...
        if not checkStatusFl('Circos_output_dir_scaffolds_{0}'.format(ClustMethod)):
            ...
            if not circoscallsScafs == []:
                with Pool(processes=procs) as p:
                    p.map(circosMultiprocessing, circoscallsScafs, chunksize=chunk_size)
                p.join()
        ...
        if not checkStatusFl('Circos_output_dir_elements_{0}'.format(ClustMethod)):
            if not circoscallsElmts == []:
                ...
                with Pool(processes=procs) as p:
                    p.map(circosMultiprocessing, circoscallsElmts, chunksize=chunk_size)
                p.join()
